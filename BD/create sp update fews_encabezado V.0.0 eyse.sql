...
/****** Object:  StoredProcedure [dbo].[AST_UPD_FEWS_ENCABEZADO_ZBC]    Script Date: 05/28/2016 02:24:06 ******/
...
into #pen
from EYSE_SERVER.FINN.dbo.AST_FEWS_LOG
...
begin
	...
	SELECT 
		...
		, p.ID_INTERFAZ
	FROM EYSE_SERVER.FINN.dbo.AST_FEWS_LOG l
	...
	FROM EYSE_SERVER.FINN.dbo.AST_FEWS_LOG_IVA
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	FROM EYSE_SERVER.FINN.dbo.AST_FEWS_LOG_TRIBUTOS
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
...
UPDATE EYSE_SERVER.FINN.dbo.AST_FEWS_LOG
SET resultado='R',
	cae=0,
...
FROM FEWS_ENCABEZADO e
	, FEWS_XML x
	, EYSE_SERVER.FINN.dbo.AST_FEWS_LOG l
	, #pen p
WHERE e.id=x.id
	and CUIT_EMPRESA=CE
	and TIPO_COMPROBANTE=TC
