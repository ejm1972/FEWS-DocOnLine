...
/****** Object:  StoredProcedure [dbo].[AST_UPD_FEWS_ENCABEZADO]    Script Date: 05/28/2016 02:24:06 ******/
...
begin
	...
	SELECT 
		...
		, p.ID_INTERFAZ
	FROM FINNEGANS.FINN_ADMIFARM.dbo.AST_FEWS_LOG l
	...
	FROM FINNEGANS.FINN_ADMIFARM.dbo.AST_FEWS_LOG_IVA
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	FROM FINNEGANS.FINN_ADMIFARM.dbo.AST_FEWS_LOG_TRIBUTOS
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
...
FROM FEWS_ENCABEZADO e
	, FEWS_XML x
	, FINNEGANS.FINN_ADMIFARM.dbo.AST_FEWS_LOG l
	, #pen p
WHERE e.id=x.id
	and CUIT_EMPRESA=CE
	and TIPO_COMPROBANTE=TC
