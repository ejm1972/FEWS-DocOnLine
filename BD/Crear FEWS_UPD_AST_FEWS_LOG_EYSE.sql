...
/****** Object:  StoredProcedure [dbo].[FEWS_UPD_AST_FEWS_LOG_EYSE]    Script Date: 12/05/2018 13:04:30 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[FEWS_UPD_AST_FEWS_LOG_EYSE]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[FEWS_UPD_AST_FEWS_LOG_EYSE]
GO
...
CREATE PROCEDURE [FEWS_UPD_AST_FEWS_LOG_EYSE]
	@id int
AS
BEGIN 
SET NOCOUNT ON
...
where fenc.id=@id
	...
	from fews_encabezado fenc
		, fews_xml fxml
		, fews_qr fqr
		, EYSELIFIN.FINN_EYSE.dbo.ast_fews_log astl
...
--exec FEWS_UPD_AST_FEWS_LOG_EYSE 1

--select * from EYSELIFIN.FINN_EYSE.dbo.ast_fews_log where cuit_empresa='30711897581' and tipo_comprobante='01' and PUNTO_VENTA='0002' and numero_comprobante='00000003'
