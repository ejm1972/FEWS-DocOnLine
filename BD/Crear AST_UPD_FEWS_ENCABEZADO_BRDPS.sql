...
/****** Object:  StoredProcedure [dbo].[AST_UPD_FEWS_ENCABEZADO_BRDPS]    Script Date: 12/05/2018 13:04:30 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[AST_UPD_FEWS_ENCABEZADO_BRDPS]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[AST_UPD_FEWS_ENCABEZADO_BRDPS]
GO
...
CREATE  PROCEDURE [dbo].[AST_UPD_FEWS_ENCABEZADO_BRDPS]  
...
from BRDPS.FINN_BRD.dbo.AST_FEWS_LOG
...
begin
	...
	SELECT 
		...
		, p.ID_INTERFAZ
	FROM BRDPS.FINN_BRD.dbo.AST_FEWS_LOG l
	...
	FROM BRDPS.FINN_BRD.dbo.AST_FEWS_LOG_IVA
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	FROM BRDPS.FINN_BRD.dbo.AST_FEWS_LOG_TRIBUTOS
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	FROM BRDPS.FINN_BRD.dbo.AST_FEWS_LOG_CBTE_ASOC
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	FROM BRDPS.FINN_BRD.dbo.AST_FEWS_LOG_DATOS_OPC
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	FROM BRDPS.FINN_BRD.dbo.AST_FEWS_LOG_PERIODO_ASOC
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	SELECT
		...
		ID_PERMISO,		
		DST_MERCADERIA		
	FROM BRDPS.FINN_BRD.dbo.AST_FEWS_LOG_PERMISO
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	SELECT
		...
		IMPORTE_TOTAL,
		BONIFICACION
	FROM BRDPS.FINN_BRD.dbo.AST_FEWS_LOG_DETALLE
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
