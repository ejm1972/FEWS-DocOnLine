USE FEWS
GO

drop table #pen
go

--Recupera lo proximo a autorizarse en AST por cada CUIT, TIPO COMPROBANTE, PUNTO VENTA
select CUIT_EMPRESA CE, TIPO_COMPROBANTE TC, PUNTO_VENTA PV, min(NUMERO_COMPROBANTE) NC, convert(int, TIPO_COMPROBANTE ) NTC, convert(int, PUNTO_VENTA) NPV, convert(int, min(NUMERO_COMPROBANTE)) NNC, 0 IDC
, (select top 1 id_interfaz from INTERFACES where CUIT_SUSCRIPCION=CUIT_EMPRESA COLLATE DATABASE_DEFAULT) ID_INTERFAZ
into #pen
from [LQ5\LQ5].FAF_HIDROTERMIA.dbo.AST_FEWS_LOG
where RESULTADO is null or RESULTADO not in ('A', 'D')
group by CUIT_EMPRESA, TIPO_COMPROBANTE, PUNTO_VENTA

select * from #pen
go

--Recupera lo proximo a autorizarse en AST por cada CUIT, TIPO COMPROBANTE, PUNTO VENTA
select *
from [LQ5\LQ5].FAF_HIDROTERMIA.dbo.AST_FEWS_LOG
where RESULTADO is null or RESULTADO not in ('A', 'D')

/*
select *
from [LQ5\LQ5].FAF_HIDROTERMIA.dbo.AST_FEWS_LOG l, #pen t
where l.CUIT_EMPRESA=CE
	and TIPO_COMPROBANTE=TC
	and PUNTO_VENTA=PV
	and NUMERO_COMPROBANTE=NC
order by TIPO_COMPROBANTE desc, PUNTO_VENTA, NUMERO_COMPROBANTE desc

select *
from [LQ5\LQ5].FAF_HIDROTERMIA.dbo.AST_FEWS_LOG_IVA l, #pen t
where l.CUIT_EMPRESA=CE
	and TIPO_COMPROBANTE=TC
	and PUNTO_VENTA=PV
	and NUMERO_COMPROBANTE=NC
order by TIPO_COMPROBANTE desc, PUNTO_VENTA, NUMERO_COMPROBANTE desc

select *
from [LQ5\LQ5].FAF_HIDROTERMIA.dbo.AST_FEWS_LOG_TRIBUTOS l, #pen t
where l.CUIT_EMPRESA=CE
	and TIPO_COMPROBANTE=TC
	and PUNTO_VENTA=PV
	and NUMERO_COMPROBANTE=NC
order by TIPO_COMPROBANTE desc, PUNTO_VENTA, NUMERO_COMPROBANTE desc

select *
from [LQ5\LQ5].FAF_HIDROTERMIA.dbo.AST_FEWS_LOG_CBTE_ASOC l, #pen t
where l.CUIT_EMPRESA=CE
	and TIPO_COMPROBANTE=TC
	and PUNTO_VENTA=PV
	and NUMERO_COMPROBANTE=NC
order by TIPO_COMPROBANTE desc, PUNTO_VENTA, NUMERO_COMPROBANTE desc

select *
from [LQ5\LQ5].FAF_HIDROTERMIA.dbo.AST_FEWS_LOG_DATOS_OPC l, #pen t
where l.CUIT_EMPRESA=CE
	and TIPO_COMPROBANTE=TC
	and PUNTO_VENTA=PV
	and NUMERO_COMPROBANTE=NC
order by TIPO_COMPROBANTE desc, PUNTO_VENTA, NUMERO_COMPROBANTE desc
*/
