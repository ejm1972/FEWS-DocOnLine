...
BEGIN
    ...
    IF @v_error = 0
            BEGIN try
				SET IDENTITY_INSERT LOG_TRANSACCION_HIS ON
				INSERT INTO LOG_TRANSACCION_HIS (ID_LOG_TRANSACCION, F_INICIO_OPERACION, F_FIN_OPERACION, ID_TIPO_TRANSACCION, XML_ENTRADA, XML_SALIDA, ID_REGISTRO_TRANSACCION, CODIGO, DESCRIPCION, OBSERVACION, EXCEPCION_WSAA, EXCEPCION_WSFEV1, RESULTADO, ERR_MSG, OBS, XML_REQUEST_AFIP, XML_RESPONSE_AFIP, TIPO_COMPROBANTE, PUNTO_VENTA, NUMERO_COMPROBANTE, FECHA_COMPROBANTE, IMPORTE_TOTAL, CAE, FECHA_VENCIMIENTO)
				select ID_LOG_TRANSACCION, F_INICIO_OPERACION, F_FIN_OPERACION, ID_TIPO_TRANSACCION, XML_ENTRADA, XML_SALIDA, ID_REGISTRO_TRANSACCION, CODIGO, DESCRIPCION, OBSERVACION, EXCEPCION_WSAA, EXCEPCION_WSFEV1, RESULTADO, ERR_MSG, OBS, XML_REQUEST_AFIP, XML_RESPONSE_AFIP, TIPO_COMPROBANTE, PUNTO_VENTA, NUMERO_COMPROBANTE, FECHA_COMPROBANTE, IMPORTE_TOTAL, CAE, FECHA_VENCIMIENTO
				from LOG_TRANSACCIONES
				where f_INICIO_OPERACION < getdate() - cast(@V_dias_depuracion as int)           
	            SET IDENTITY_INSERT LOG_TRANSACCION_HIS OFF
...
BEGIN
    ...
    IF @v_error = 0
        BEGIN try
			SET IDENTITY_INSERT REGISTRO_TRANSACCION_HIS ON
            INSERT INTO REGISTRO_TRANSACCION_HIS (ID_REGISTRO_TRANSACCION, ID_INTERFAZ, F_TRANSACCION, COD_TRANSACCION, DATOS_TRANSACCION, SERVICIO)
            select ID_REGISTRO_TRANSACCION, ID_INTERFAZ, F_TRANSACCION, COD_TRANSACCION, DATOS_TRANSACCION, SERVICIO
            from REGISTROS_TRANSACCIONES
            where F_TRANSACCION < getdate() -  cast(@V_dias_depuracion as int)           
            SET IDENTITY_INSERT REGISTRO_TRANSACCION_HIS OFF
