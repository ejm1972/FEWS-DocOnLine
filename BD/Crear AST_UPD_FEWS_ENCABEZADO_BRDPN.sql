...
/****** Object:  StoredProcedure [dbo].[AST_UPD_FEWS_ENCABEZADO_BRDPN]    Script Date: 12/05/2018 13:04:30 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[AST_UPD_FEWS_ENCABEZADO_BRDPN]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[AST_UPD_FEWS_ENCABEZADO_BRDPN]
GO
...
CREATE  PROCEDURE [dbo].[AST_UPD_FEWS_ENCABEZADO_BRDPN]  
...
from BRD3.FINN_BRD.dbo.AST_FEWS_LOG
...
begin
	...
	SELECT 
		...
		, p.ID_INTERFAZ
	FROM BRD3.FINN_BRD.dbo.AST_FEWS_LOG l
	...
	FROM BRD3.FINN_BRD.dbo.AST_FEWS_LOG_IVA
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	FROM BRD3.FINN_BRD.dbo.AST_FEWS_LOG_TRIBUTOS
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	FROM BRD3.FINN_BRD.dbo.AST_FEWS_LOG_CBTE_ASOC
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	FROM BRD3.FINN_BRD.dbo.AST_FEWS_LOG_DATOS_OPC
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	FROM BRD3.FINN_BRD.dbo.AST_FEWS_LOG_PERIODO_ASOC
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	SELECT
		...
		ID_PERMISO,		
		DST_MERCADERIA		
	FROM BRD3.FINN_BRD.dbo.AST_FEWS_LOG_PERMISO
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	SELECT
		...
		IMPORTE_TOTAL,
		BONIFICACION
	FROM BRD3.FINN_BRD.dbo.AST_FEWS_LOG_DETALLE
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
...
UPDATE BRD3.FINN_BRD.dbo.AST_FEWS_LOG
SET resultado='D',
	cae=e.cae,
	qr=fqr.imagen_qr,
...
FROM FEWS_ENCABEZADO e
	, FEWS_XML x
	, fews_qr fqr
	, BRD3.FINN_BRD.dbo.AST_FEWS_LOG l
	, #pen p
WHERE e.id=x.id
	and CUIT_EMPRESA=CE
	and TIPO_COMPROBANTE=TC
