...
/****** Object:  StoredProcedure [dbo].[AST_UPD_FEWS_ENCABEZADO_TMP]    Script Date: 12/27/2018 10:53:10 ******/
...
begin
	...
	FROM TMP_FINN.dbo.AST_FEWS_LOG_CBTE_ASOC
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	FROM TMP_FINN.dbo.AST_FEWS_LOG_DATOS_OPC
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
...
UPDATE TMP_FINN.dbo.AST_FEWS_LOG
SET resultado='D',
	cae=0,
