...
into #pen
from MACOR_SERVER.FINN_MACOR2014.dbo.AST_FEWS_LOG
...
begin
	...
	SELECT 
		...
		, p.ID_INTERFAZ
	FROM MACOR_SERVER.FINN_MACOR2014.dbo.AST_FEWS_LOG l
	...
	FROM MACOR_SERVER.FINN_MACOR2014.dbo.AST_FEWS_LOG_IVA
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	FROM MACOR_SERVER.FINN_MACOR2014.dbo.AST_FEWS_LOG_TRIBUTOS
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	FROM MACOR_SERVER.FINN_MACOR2014.dbo.AST_FEWS_LOG_CBTE_ASOC
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
	...
	FROM MACOR_SERVER.FINN_MACOR2014.dbo.AST_FEWS_LOG_DATOS_OPC
		, #pen p left outer join FEWS_ENCABEZADO e on e.cuit=p.CE COLLATE DATABASE_DEFAULT and e.tipo_cbte=p.NTC and e.punto_vta=p.NPV and e.cbte_nro=p.NNC
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
		and PUNTO_VENTA=PV
...
UPDATE MACOR_SERVER.FINN_MACOR2014.dbo.AST_FEWS_LOG
SET resultado='D',
	cae=0,
...
FROM FEWS_ENCABEZADO e
	, FEWS_XML x
	, MACOR_SERVER.FINN_MACOR2014.dbo.AST_FEWS_LOG l
	, #pen p
WHERE e.id=x.id
	and CUIT_EMPRESA=CE
	and TIPO_COMPROBANTE=TC
