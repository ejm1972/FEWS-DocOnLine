...
/****** Object:  StoredProcedure [dbo].[AST_UPD_FEWS_ENCABEZADO_tmp]    Script Date: 06/06/2016 11:21:09 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[AST_UPD_FEWS_ENCABEZADO_tmp]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[AST_UPD_FEWS_ENCABEZADO_tmp]
GO
...
CREATE PROCEDURE [dbo].[AST_UPD_FEWS_ENCABEZADO_tmp]
AS    
BEGIN
SET NOCOUNT ON
...
--Bloque Agregado de pendiente con Interfaz Válida
begin
	...
	SELECT 
		...
		, p.ID_INTERFAZ
	FROM TMP_ADMIFARM.dbo.AST_FEWS_LOG l
...
--Actualizo Error de los encabezados con Interfaz Inválida
begin
	UPDATE TMP_ADMIFARM.dbo.AST_FEWS_LOG
	SET resultado='R',
		cae=0,
		err_msg='ERROR - COMPROBANTE PENDIENTE CON INTERFAZ INVALIDA - VERIFIQUE EL CUIT - EMPRESA - INTERFAZ',
		obs='',
		codigo='14003',
		descripcion='ERROR - COMPROBANTE PENDIENTE CON INTERFAZ INVALIDA - VERIFIQUE EL CUIT - EMPRESA - INTERFAZ'
	FROM TMP_ADMIFARM.dbo.AST_FEWS_LOG l
		, #pen p
	WHERE CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
...
--Actualizo Error de los encabezados que existen
begin
	UPDATE TMP_ADMIFARM.dbo.AST_FEWS_LOG
	SET resultado='R',
		cae=0,
	...
	FROM FEWS_ENCABEZADO e
		, FEWS_XML x
		, TMP_ADMIFARM.dbo.AST_FEWS_LOG l
		, #pen p
	WHERE e.id=x.id
		and CUIT_EMPRESA=CE
		and TIPO_COMPROBANTE=TC
